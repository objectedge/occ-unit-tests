...
const updateOracleJet = require('./grab-oracle-jet');
...
const oracleLibsDir = path.join(configs.application.oracleLibsDir);
const url = configs.application.occStoreUrl;
const httpAuth = "Basic " + new Buffer(configs.application.HTTPAuthCredentials.user + ":" + configs.application.HTTPAuthCredentials.password).toString("base64");
const allDependencies = [];

fs.removeSync(oracleLibsDir);
fs.ensureDirSync(oracleLibsDir);
...
const makeRequest = (url, callback) => {
  ...
  if(configs.application.HTTPAuth) {
...
function grabMissingDependencies() {
  ...
  let mainJSFile;
  try {
    mainJSFile = fs.readFileSync(path.join(oracleLibsDir, 'main.js'), 'utf8');
  } catch(error) {
    console.log(`Error on trying the read the main.js file, please, run the command "yarn update-shared-libs" in order to get the ${path.join(oracleLibsDir, 'main.js')}`);
    process.exit(0);
  }
  ...
  if(!Object.keys(requireJSPaths).length) {
    console.log(`No Oracle Paths found in ${path.join(oracleLibsDir, 'main.js')}`);
    return;
  }
